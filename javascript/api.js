...
//Initail set up off the languages to be used for translation
var languages = [{//1
  language: "Espanol",
  value: "en-es"
}, {//2
...
$(document).on("click", '.answer-button1', function(e) {
	...
	$("#panel").empty();
//Set variable to API url to translate the text from the question to selected language by user
...
//AJAX to recieve the response from the api
	$.ajax({url: queryUrl, method : 'GET'})
		.done(function(response){
			var result = response.text;
			
      //displaying the results of the call on to the HTML
...
var queryUrl5 = 'https:translate.yandex.net/api/v1.5/tr.json/translate?key=trnsl.1.1.20160317T225805Z.24fe4d3e49b00f5f.6bcc824a7ac0b4ec0da91a97f022d7045d9b121c&text=' +  questions[questionNumber].answers[3] +  '&lang=' + $(event.target).attr("value");
		...
		.done(function(response){
			var result = response.text;
				 ...
				 $("#panel").append("<button class='answer-button2' value='test' data-name='" + questions[questionNumber].answers[3] + "' >" + response.text[0] + "</button>");
